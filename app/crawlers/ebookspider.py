...
from app import db
from app.crawlers.novelsql import get_list, novels, SqlDB
from app.models import Ebooks
...
class EbookSpider:
    ...
    def get_allbook_url(self):
        '''
        获取e笔趣阁
        :return: 首页所有书链接，保存到mongodb all_book集合
        '''
        try:
            ...
            print(e.args)
            self.get_allbook_url()

    def get_chapter_url(self, book_url, book_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.data['book_name'] = book_name
                self.data['author'] = author
                self.data['book_image'] = None
                self.data['last_since'] = last_since
                self.data['new_chapter'] = new_chapter
                self.data['about_book'] = about_book
                self.data['book_url'] = book_url
                print(self.data)
                # 章节
                list_name = author + '-' + book_name
                list_name_db = get_list(list_name)
                self.sql.insert_data(self.data)
                # fail_book_db
                num = 1
                ...
                success_book.insert_one(self.data)
                all_book = get_list('all_book')
                all_book.delete_one({ 'book_name': book_name,
                                      'book_url': book_url})
            ...
            fail_book_db.insert_one(temp_data)
            # print(book_name, '爬取目录链接失败------------------------')
        except Exception as e:
            print(e.args)
            print(book_name, '已爬取')

    def get_chapter_content(self, chapter_success_list, chapter_fail_list, chapter_url, id, chapter_name):
    ...
    def srart_spider(self):
        ...
        for book in all_book_db.find():
            ...
            author = self.get_chapter_url(book_url, book_name)
            ...
            for bb in novels.list_collection_names():
                ...
                for col in book_db.find():
                    ...
                    self.get_chapter_content(chapter_success_list, chapter_fail_list, chapter_url, id, chapter_name)
