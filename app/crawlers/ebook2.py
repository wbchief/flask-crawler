...
def parse_detals():
    ...
    # 章节
    print(name)
    chapter_url = [base_url + x.attr('href') for x in doc('div.box_con div#list > dl > dd:gt(11) > a').items()]
    for x in doc('div.box_con div#list > dl > dd:gt(11) > a').items():
        sheet_words.insert_one({'_id': 1, 'chapter_url': base_url + x.attr('href')})
    print(chapter_url)
    return chapter_url, name
